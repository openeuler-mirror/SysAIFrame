...
class LowestLatencyStrategy(BaseRoutingStrategy):
    ...
    def __init__(self, config_manager=None):
        ...
        # Track latency history per model instance_id
        # Format: instance_id -> deque of recent latencies
        self._latency_history: Dict[str, deque] = {}
        self._lock = threading.Lock()
        
        # Get latency window size from config
        if config_manager:
        ...
        with self._lock:
            ...
            for model in healthy_models:
                ...
                if len(history) > 0:
                    ...
                    # No history yet, use a high default to prefer models with history
            ...
            # Apply latency buffer: select from models within buffer range of minimum
            min_latency = latencies[0][1]
            if min_latency == float('inf'):
                # No history for any model, select first
                selected = healthy_models[0]
            else:
                # Find models within buffer range
                ...
                # Randomly select from candidates (to avoid always picking same model)
                import random
